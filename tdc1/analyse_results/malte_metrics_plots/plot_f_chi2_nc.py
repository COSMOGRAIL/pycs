...
sels = ["_d3cs-vanilla-dou-full", "_spl-vanilla-dou-full", "_sdi-vanilla-dou-full"]
...
for team in teams:
	...
	x = np.log10(seldf["chi2-nc"].values)
	y = seldf["f-nc"].values
	c = np.abs(seldf["A-nc"].values)
	#print sel, len(x) 
	...
	#sc = plt.scatter(x=x, y=y, c=c, s=100, vmin=0.0001, vmax=1.0, marker=next(marker), edgecolor="None", label=team, norm=matplotlib.colors.LogNorm())
	...
	for (i, sub) in seldf.iterrows():
		s = "%i" % (sub["rung"])
		pos = np.array([np.log10(sub["chi2-nc"]), sub["f-nc"]])
		plt.annotate(s,
			pos, xycoords='data', 
...
plt.ylim(0, 1)
cb = plt.colorbar()
cb.set_label(r'|A-nc|', fontsize=26)
plt.xlabel(r"log$_{10} (\chi^2-nc)$", fontsize=26)
plt.ylabel("f-nc", fontsize=26)
