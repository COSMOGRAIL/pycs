...
#for sel in sels:
	...
	x = seldf["chi2-nc"].values
	xerr = seldf["chi2-ncerr"].values
	y = np.abs(seldf["A-nc"].values)
	yerr = seldf["A-ncerr"].values
	c = seldf["P-nc"].values
	#print sel, len(x) 
	...
	sc = plt.scatter(x=x, y=y, c=c, s=120, vmin=0.001, vmax=1.0, marker=next(marker), zorder = 2, label=team, norm=matplotlib.colors.LogNorm())
	...
	for (i, sub) in seldf.iterrows():
		...
		#print offset
		pos = np.array([sub["chi2-nc"], abs(sub["A-nc"])])
		plt.annotate(s,
			pos, xycoords='data', 
			xytext=(5,5), textcoords='offset points', fontsize=12, bbox=dict(boxstyle="round,pad=0.3", fc="gray", alpha=0.1, ec="b", lw=1))
...
plt.ylim(3e-5, 1e-1)
cb = plt.colorbar()
cb.set_label(r'$P$-nc', fontsize=26)
plt.xlabel(r"$\chi^2$-nc", fontsize=26)
plt.ylabel(r"$|A|$-nc", fontsize=26)
