...
subnames = ["pycs_tdc1_d3cs-vanilla-dou-full", "d3cs_Vivien","d3cs_mtewes","d3cs_Fred","d3cs_Thibault"]
...
for subi, subname in enumerate(subnames):
	...
	#seldb = [item for item in db if "pycs_tdc1_d3cs-vanilla-dou-full_td" in item and subname + "_td" in item]
	
	seldb = [item for item in db if subname + "_td" in item and abs(item[subname + "_td"] - item["truetd"]) < 5.0]
	seldb = [item for item in seldb if item.get(subname + "_confidence", 1) <= 2]
