...
subname = "pycs_tdc1_spl-median-doupla-splsdiagree"
...
for (sub_td_item, sub_tderr_item, d3cs_td_item) in zip(sub_td, sub_tderr, d3cs_td):
	if abs(sub_td_item - d3cs_td_item) > 10.0:
		print "outlier:", sub_td_item, sub_tderr_item, d3cs_td_item
...
plt.axvline(np.mean(sub_reltderr),color='indigo',linestyle='dotted',linewidth=2,label='mean : %.2f'%np.mean(sub_reltderr))
plt.legend()
