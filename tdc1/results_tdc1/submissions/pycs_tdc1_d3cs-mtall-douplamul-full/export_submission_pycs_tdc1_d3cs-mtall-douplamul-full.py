...
subname  = "pycs_tdc1_d3cs-mtall-douplamul-full"
filepath = os.path.join("results_tdc1",'submissions',subname,subname+'.dt')
dirpath = os.path.dirname(filepath)
commentlist = ["WARNING: this contains -99 for all curves, even those not analyzed!"]
...
for est in estimates:
	if est.confidence not in [1,2,3]: 
		est.td = 99.00
		est.tderr = -99.00
...
cleanestimates = [est for est in estimates if est.confidence in [1,2,3]]
...
pycs.tdc.util.writesubmission(estimates, filepath, commentlist, theseonly = False) # And write only the current estimates
